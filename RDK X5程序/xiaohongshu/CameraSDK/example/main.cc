...
int main(int argc, char* argv[]) {
    ...
    std::cout << "6: start normal recording" << std::endl;
    std::cout << "7: stop normal recording" << std::endl;
    std::cout << "8: start timelapse " << std::endl;
    std::cout << "9: stop timelapse " << std::endl;
    std::cout << "40: start timeshift " << std::endl;
    std::cout << "41: stop timeshift " << std::endl;
    std::cout << "42: start PureVide capture " << std::endl;
    std::cout << "43: stop PureVide capture" << std::endl;
    std::cout << "44: start HDRVideo capture " << std::endl;
    std::cout << "45: stop HDRVideo capture" << std::endl;
    std::cout << "46: set global sharpness" << std::endl;
    std::cout << "47: set global sharpness" << std::endl;
    std::cout << "48: set photo timer" << std::endl;
    std::cout << "49: start intelval photo" << std::endl;
    std::cout << "50: stop intelval photo" << std::endl;
    ...
    cam->SyncLocalTimeToCamera(now, time_seconds - now);
    ...
    //record_params_test.resolution = ins_camera::VideoResolution::RES_57P30;
    ...
    //auto param = cam->GetPreviewParam();

    cam->SetCaptureStateNotification([](bool is_capture)
    {
        std::cout << "is_capture: " << is_capture << std::endl;
    });
    ...
    while (true) {
        ...
        if (option == 5) {
            ...
            std::cout << "PHOTO_BURST photo [3]" << std::endl;
        ...
        if (option == 6) {
            ...
            record_params.resolution = ins_camera::VideoResolution::RES_8KP30;
        ...
        if (option == 8) {
            ...
            // 6k    RES_6KP30
            // X4支持11K30, X3不支持, X4Air只是支持8k
            std::vector<ins_camera::VideoResolution> support_list = { ins_camera::VideoResolution::RES_8KP30 };
            if (camera_type < ins_camera::CameraType::Insta360X4) {
                support_list.push_back(ins_camera::VideoResolution::RES_57KP30);
            }
            else if (camera_type == ins_camera::CameraType::Insta360X4) {
                support_list.push_back(ins_camera::VideoResolution::RES_11KP30);
                support_list.push_back(ins_camera::VideoResolution::RES_57KP30);
            }

            static int count = 0;
            ins_camera::RecordParams record_params;
            record_params.resolution = support_list[(count++) % support_list.size()];
        ...
        if (option == 10) {
            // 对于X5相机 设置预览流 RES_3840_1920P30, RES_2560_1280P30, RES_2160_1080P30, RES_1920_960P30
            // 对于X4相机 设置预览流
            ...
            std::cout << "rolling_shutter_time: " << preview_param.sweep_time << std::endl;
        }
        ...
        if (option == 12) {
            if (camera_type < ins_camera::CameraType::Insta360X4) {
                std::cout << "In-camera stitching is only supported on X4/X4Air/X5" << std::endl;
                continue;
            }
            int enable = 0;
        ...
        if (option == 19) {
            ...
            if (!fileIsExist(file_to_save_dir)) {
                std::cout << "not exist: " << file_to_save_dir << std::endl;
                continue;
            }
        ...
        if (option == 25) {
            ...
            const auto url_list = url.OriginUrls();
        ...
        if (option == 32) {
            ...
            if (camera_type == ins_camera::CameraType::Insta360X4Air) {
                // only support 8K
                record_params.resolution = ins_camera::VideoResolution::RES_8KP30;
            }
            record_params.bitrate = { 1024 * 1024 * 10 };
        ...
        if (option == 34) {
            std::vector<ins_camera::PhotoSize> photo_size_vec = { ins_camera::PhotoSize::Size_72M ,ins_camera::PhotoSize::Size_X4_18MP };
            if (camera_type < ins_camera::CameraType::Insta360X4) {
                photo_size_vec = { ins_camera::PhotoSize::Size_72M ,ins_camera::PhotoSize::Size_X3_18MP };
            }
            if (camera_type == ins_camera::CameraType::Insta360X4Air) {
                photo_size_vec = { ins_camera::PhotoSize::Size_X4Air_29MP };
            }

            static int hdr_count = 0;
            auto urls = cam->StartHDRCapture(photo_size_vec[(hdr_count++) % photo_size_vec.size()]);
            if (urls.Empty()) {
                std::cout << "failed to take picture" << std::endl;
            }
            else {
        ...
        if (option == 40) {
            bool ret = cam->SetVideoSubMode(ins_camera::SubVideoMode::VIDEO_TIMESHIFT);
            if (!ret) {
                std::cout << "change sub mode failed!" << std::endl;
                continue;
            }

            std::vector<int> support_accelerate_fequency = { 0,10,15,20,30,60 };
            int accelerate_fequency;
            std::cout << "[Auto -->(0) ]" << std::endl;
            std::cout << "[10x --->(10)]" << std::endl;
            std::cout << "[15x --->(15)]" << std::endl;
            std::cout << "[20x --->(20)]" << std::endl;
            std::cout << "[30x --->(30)]" << std::endl;
            std::cout << "[60x --->(60)]" << std::endl;
            std::cout << "please accelerate fequency: ";
            std::cin >> accelerate_fequency;
            if (std::find(support_accelerate_fequency.begin(), support_accelerate_fequency.end(), accelerate_fequency) == support_accelerate_fequency.end()) {
                std::cerr << "The input accelerate fequency is not supported" << std::endl;
                continue;
            }
            ...
            record_params.resolution = ins_camera::VideoResolution::RES_57KP30;
            if (camera_type == ins_camera::CameraType::Insta360X4Air) {
                record_params.resolution = ins_camera::VideoResolution::RES_6KP30;
            }
            record_params.accelerate_fequency = accelerate_fequency;
            ret = cam->SetVideoCaptureParams(record_params, ins_camera::CameraFunctionMode::FUNCTION_MODE_TIMESHIFT);
            if (!ret) {
                std::cout << "failed to set video capture param";
                continue;
            }
            ...
            ret = cam->StartRecording();
            if (ret) {
                std::cout << "start timeshift succeed!" << std::endl;
            }
            else {
                std::cout << "failed to start timeshift failed!" << std::endl;
            }
        }
        ...
        if (option == 42) {
            ...
            bool ret = cam->SetVideoSubMode(ins_camera::SubVideoMode::VIDEO_PURE);
            if (!ret) {
                std::cout << "change sub mode failed!" << std::endl;
                continue;
            }
            ...
            ins_camera::RecordParams record_params;
            record_params.resolution = ins_camera::VideoResolution::RES_57KP30;
            ret = cam->SetVideoCaptureParams(record_params, ins_camera::CameraFunctionMode::FUNCTION_MODE_PURE_VIDEO);
            if (!ret) {
                std::cout << "failed to set video capture param";
                continue;
            }
            ...
            ret = cam->StartRecording();
            if (ret) {
                std::cout << "start pure video succeed!" << std::endl;
            }
            else {
                std::cout << "failed to start pure video failed!" << std::endl;
            }
        }
        ...
        if (option == 44) {
            bool ret = cam->SetVideoSubMode(ins_camera::SubVideoMode::VIDEO_HDR);
            if (!ret) {
                std::cout << "change sub mode failed!" << std::endl;
                continue;
            }
            ...
            ret = cam->SetVideoCaptureParams(record_params, ins_camera::CameraFunctionMode::FUNCTION_MODE_HDR_VIDEO);
            if (!ret) {
                std::cout << "failed to set video capture param";
                continue;
            }
            ...
            ret = cam->StartRecording();
            if (ret) {
                std::cout << "start hdr video succeed!" << std::endl;
            }
            else {
                std::cout << "failed to start hdr video failed!" << std::endl;
            }
        }
        ...
        if (option == 46) {
            ...
            std::cout << "[Media --->(1)]" << std::endl;
            std::cout << "[Hight --->(2)]" << std::endl;
            std::cout << "please global sharpness level: ";
            ...
            if (!cam->SetGlobalSharpness(static_cast<ins_camera::SharpnessLevel>(level))) {
                std::cout << "failed to set global sharpness level" << std::endl;
            }
            else {
                std::cout << "set global sharpness level succeed" << std::endl;
            }
        }

        if (option == 47) {
            ins_camera::SharpnessLevel level;
            if (!cam->GetGlobalSharpness(level)) {
                std::cout << "failed to set global sharpness level" << std::endl;
            }
            else {
                if (level == ins_camera::SharpnessLevel::Low) {
                    std::cout << "Global sharpness level: Low" << std::endl;
                }
                else if (level == ins_camera::SharpnessLevel::Medium) {
                    std::cout << "Global sharpness level: Media" << std::endl;
                }
                else if (level == ins_camera::SharpnessLevel::High) {
                    std::cout << "Global sharpness level: High" << std::endl;
                }
            }
        }
        ...
        if (option == 48) {
            ...
            bool ret = cam->SetPhotoSubModeTimer(static_cast<ins_camera::SubPhotoMode>(photo_mode), duration);
            if (ret) {
                std::cout << "SetPhotoSubModeTimer Succeed!" << std::endl;
            }
            else {
                std::cout << "SetPhotoSubModeTimer Failed!" << std::endl;
            }
        }

        if (option == 49) {
            bool ret = false;
            uint32_t lapseTime = 3000; //单位毫秒
            ins_camera::TimelapseParam param = { ins_camera::CameraTimelapseMode::TIMELAPSE_INTERVAL_SHOOTING, 0,lapseTime,5 };
            if (!cam->SetTimeLapseOption(param)) {
                std::cerr << "failed to set capture settings." << std::endl;
            }
            else {
                ret = cam->StartTimeLapse(ins_camera::CameraTimelapseMode::TIMELAPSE_INTERVAL_SHOOTING);
                if (ret) {
                    std::cerr << "success!" << std::endl;
                }
                else {
        ...
        if (option == 50) {
            auto url = cam->StopTimeLapse(ins_camera::CameraTimelapseMode::TIMELAPSE_INTERVAL_SHOOTING);
            if (url.Empty()) {
                std::cerr << "stop timelapse failed" << std::endl;
                continue;
            }
